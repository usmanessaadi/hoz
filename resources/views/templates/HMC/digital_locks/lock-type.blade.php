<div id="s-c">
    <div class="hmc_choices d-flex raw">
        @foreach($getDigitalLock_LocksType as $locktype)
            <div class="choice d-flex flex-column align-items-center choice-lock-type"
                 data-url="{{Route('get-dl-handles-type',[$selected_doorType,$locktype->id,])}}"
                 data-doorType="{{$selected_doorType}}"
                 data-lockType="{{$locktype->id}}">
                <a class="choice_img" href="#">
                    {{--<h5>doors type</h5>--}}
                    <img src="{{asset($locktype->icon)}}">
                </a>
                ...
                <span>{{$locktype->name}}</span>
            </div>
        @endforeach
        {{--disabled options--}}
        @foreach($getAvailable_DigitalLock_LocksType_Excluding_OutOfStock_And_TrueData as $locktype)
                <div class="choice d-flex flex-column align-items-center choice-lock-type"
                     data-url="{{Route('get-dl-handles-type',[$selected_doorType,$locktype->id,])}}"
                     data-doorType="{{$selected_doorType}}"
                     data-lockType="{{$locktype->id}}">
    ...
    <script>
        ...
        $('.choice-lock-type').click(function (e) {
            e.preventDefault();
            ...
            help_me_choose(selected_door_type, selected_lock_type, selected_choice_url);
            // $(this).addClass('active_type');
            // $(this).siblings().removeClass('active_type')
        });


        function help_me_choose(selected_door_type,selected_lock_type, url) {
