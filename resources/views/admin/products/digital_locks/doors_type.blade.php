        ...
        <div class="col-6">
            ...
            <div class="card">
                ...
                <h5 class="card-header info-color white-text text-center py-4">
                    <strong>Add new Digital doors type </strong>
                </h5>
                ...
                <div class="card-body px-lg-5 pt-0">
                    ...
                    <!-- Form -->
                    <form class="text-center" style="color: #757575;" action="{{Route('add-digital-locks-type','doors_type')}}" method="POST">
                        @csrf
                        <div class="row">
                            <div class="col-6">
                                <!-- Name -->
...
<table id="dtBasicExample" class="table table-striped table-bordered table-sm" cellspacing="0" width="100%">
    ...
    <tbody>
        @foreach ($digital_doors_type as $door_type)
            <tr>
                 <td>#{{$door_type->id}}</td>
                 <td>{{$door_type->name}}</td>
                 <td>{{$door_type->digital_lock()->count()}}</td>
                 <td>
                    @if ($door_type->digital_lock()->count())
                    <button type="button" disabled class="btn btn-danger" style="margin:0;margin-left:6px; padding: 4px 11px;"> {{__('Delete')}}</button>
                       @else
                       <form action="{{Route('delete-digital-locks-type',['type'=>'doors_type','id'=>$door_type->id])}}" method="POST" style="display: inline;">
                        @csrf
                        {{ method_field('DELETE') }}
                   ...
                   <button type="button" class="btn btn-primary edit-categ"
                           ...
                           data-name="{{$door_type->name}}"
                           data-url="{{Route('update-digital-locks-type',['type'=>'doors_type','id'=>$door_type->id])}}"
