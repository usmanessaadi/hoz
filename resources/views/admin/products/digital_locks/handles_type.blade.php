        ...
        <div class="col-6">
            ...
            <div class="card">
                ...
                <h5 class="card-header info-color white-text text-center py-4">
                    <strong>Add new Digital handles type </strong>
                </h5>
                ...
                <div class="card-body px-lg-5 pt-0">
                    ...
                    <!-- Form -->
                    <form class="text-center" style="color: #757575;" action="{{Route('add-digital-locks-type','handles_type')}}" method="POST">
                        @csrf
                        <div class="row">
                            <div class="col-6">
                                <!-- Name -->
...
<table id="dtBasicExample" class="table table-striped table-bordered table-sm" cellspacing="0" width="100%">
    ...
    <tbody>
        @foreach ($digital_handles_type as $handle_type)
            <tr>
                 <td>#{{$handle_type->id}}</td>
                 <td>{{$handle_type->name}}</td>
                 <td>{{$handle_type->digital_lock()->count()}}</td>
                 <td>
                    @if ($handle_type->digital_lock()->count())
                    <button type="button" disabled class="btn btn-danger" style="margin:0;margin-left:6px; padding: 4px 11px;"> {{__('Delete')}}</button>
                       @else
                       <form action="{{Route('delete-digital-locks-type',['type'=>'handles_type','id'=>$handle_type->id])}}" method="POST" style="display: inline;">
                        @csrf
                        {{ method_field('DELETE') }}
                   ...
                   <button type="button" class="btn btn-primary edit-categ"
                           ...
                           data-name="{{$handle_type->name}}"
                           data-url="{{Route('update-digital-locks-type',['type'=>'handles_type','id'=>$handle_type->id])}}"
